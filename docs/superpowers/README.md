...
| C | tool 境界（`sh -lc` 廃止、`.git`/`.ssh` の case-fold、TOCTOU 再検証） | 未着手 | — |
...
**Spec C** が次です。承認済み `shell_command` はまだ `System.cmd("sh", ["-lc", command])` です。argv 直実行へ替えるのが、承認ゲートの後ろに残っている実行面の穴です。
